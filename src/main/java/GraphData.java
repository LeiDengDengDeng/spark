...
public class GraphData {
    ...
    public void calRate() {
        String degrees = FileUtil.readString("degrees.txt");
        Map<Long, Integer> sumMap = new HashMap<>();
        for (String line : degrees.split("\n")) {
            sumMap.put(Long.parseLong(line.split(",")[0]), Integer.parseInt(line.split(",")[1]));
        }

        String links = FileUtil.readString("links.txt");
